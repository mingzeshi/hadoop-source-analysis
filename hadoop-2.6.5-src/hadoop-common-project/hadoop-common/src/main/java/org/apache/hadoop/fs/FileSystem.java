...
public abstract class FileSystem extends Configured implements Closeable {
  ...
  protected void checkPath(Path path) {
    URI uri = path.toUri(); // 文件路径的URI
    String thatScheme = uri.getScheme();
    if (thatScheme == null)                // fs is relative
      return;
    URI thisUri = getCanonicalUri(); // 拿到：file:///、hdfs://
    String thisScheme = thisUri.getScheme(); // 拿到：file、hdfs
    //authority and scheme are not case sensitive
    if (thisScheme.equalsIgnoreCase(thatScheme)) {// 当前文件系统(FileSystem)的URI与文件路径的URI相匹配
    ...
    throw new IllegalArgumentException("Wrong FS: "+path+
                                       ", expected: "+this.getUri()); // 只要不抛异常，正常返回就行
  }
