...
public class Cluster {
  ...
  public Cluster(Configuration conf) throws IOException { // 创建cluster
    this(null, conf);
  }

  public Cluster(InetSocketAddress jobTrackAddr, Configuration conf) 
      throws IOException {
    this.conf = conf; // job的configuration
    this.ugi = UserGroupInformation.getCurrentUser(); // 获取ugi
    initialize(jobTrackAddr, conf); // 初始化cluster
  }
  
  private void initialize(InetSocketAddress jobTrackAddr, Configuration conf)
      throws IOException {

    synchronized (frameworkLoader) { // 加载器
      /**
       *  org.apache.hadoop.mapred.YarnClientProtocolProvider
       *  org.apache.hadoop.mapred.LocalClientProtocolProvider
       */
      for (ClientProtocolProvider provider : frameworkLoader) { // 循环多个ClientProtocolProvider，直到可以匹配创建相应的ClientProtocol
        LOG.debug("Trying ClientProtocolProvider : "
            + provider.getClass().getName());
        ClientProtocol clientProtocol = null; 
        try {
          if (jobTrackAddr == null) { // 刚刚提交任务的时候，jobTrackAddr是null
            clientProtocol = provider.create(conf); // 根据配置conf(加载的core-site.xml等)；尝试创建clientProtocol，就是与集群通信的客户端：YarnClientProtocol、LocalClientProtocol
          } else {
            clientProtocol = provider.create(jobTrackAddr, conf);
          }

          if (clientProtocol != null) { // 如果成功创建clientProtocol，则赋成员变量；否则继续迭代遍历frameworkLoader，直到可以成功创建clientProtocol为止
            clientProtocolProvider = provider; // ClientProtocolProvider赋值给clientProtocolProvider
            client = clientProtocol; // 成功创建的clientProtocol赋值给client，是与集群通信的组件
  ...
  public synchronized FileSystem getFileSystem() 
    ...
    if (this.fs == null) { // fileSystem为null
      try {
        this.fs = ugi.doAs(new PrivilegedExceptionAction<FileSystem>() {
          public FileSystem run() throws IOException, InterruptedException {
            ...
            return sysDir.getFileSystem(getConf()); // 拿到文件系统
          }
        });
  ...
  public Path getStagingAreaDir() throws IOException, InterruptedException {
    if (stagingAreaDir == null) { // 如果job存放临时文件的文件夹不存在，则创建
      stagingAreaDir = new Path(client.getStagingAreaDir());
    }
    return stagingAreaDir; // 返回任务的临时文件存放路径
  }
