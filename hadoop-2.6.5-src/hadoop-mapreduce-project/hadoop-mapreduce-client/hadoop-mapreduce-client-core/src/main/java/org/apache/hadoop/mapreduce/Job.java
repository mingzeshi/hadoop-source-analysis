...
public class Job extends JobContextImpl implements JobContext {  
  ...
  private void setUseNewAPI() throws IOException {
    int numReduces = conf.getNumReduceTasks(); // reduce任务数量，默认是1个
    String oldMapperClass = "mapred.mapper.class";
    String oldReduceClass = "mapred.reducer.class";
    conf.setBooleanIfUnset("mapred.mapper.new-api",
                           conf.get(oldMapperClass) == null);
    if (conf.getUseNewMapper()) { // 表示使用新的API组件
    ...
    if (numReduces != 0) { // 如果reduce的数量不为0，设置reduce使用的API组件
      conf.setBooleanIfUnset("mapred.reducer.new-api",
                             conf.get(oldReduceClass) == null);
      if (conf.getUseNewReducer()) { // 如果reduce使用新的API组件
  ...
  private synchronized void connect()
    ...
    if (cluster == null) { // 刚刚提交任务的时候，cluster是null
      cluster = // 创建任务运行组件，如果是集群模式提交，是YarnRunner；本地提交是LocalRunner
  ...
  public void submit() 
         throws IOException, InterruptedException, ClassNotFoundException {
    ensureState(JobState.DEFINE); // 检查任务状态
    setUseNewAPI(); // 设置一些数据处理组件
    connect(); // 连接cluster
    final JobSubmitter submitter = // 拿到提交job的submitter
        getJobSubmitter(cluster.getFileSystem(), cluster.getClient()); // cluster.getFileSystem()：获取文件系统；获取与集群通信的client，任务运行runner
    status = ugi.doAs(new PrivilegedExceptionAction<JobStatus>() { // 提交任务，并返回提交状态
      public JobStatus run() throws IOException, InterruptedException, 
      ClassNotFoundException {
        return submitter.submitJobInternal(Job.this, cluster); // 并传入cluster，与集群通信；submitter提交job并返回任务运行状态
      }
    });
    state = JobState.RUNNING;
  ...
  public boolean waitForCompletion(boolean verbose
                                            ...
                                            ClassNotFoundException {
    if (state == JobState.DEFINE) { // state：任务状态；初始化状态：JobState.DEFINE提交任务
      submit();
    }
    if (verbose) {
      monitorAndPrintJob(); // 输出任务运行日志
    } else {
