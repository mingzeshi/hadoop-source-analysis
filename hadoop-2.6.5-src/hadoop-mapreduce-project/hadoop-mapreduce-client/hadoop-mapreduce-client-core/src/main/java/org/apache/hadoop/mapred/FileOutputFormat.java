...
public abstract class FileOutputFormat<K, V> implements OutputFormat<K, V> {
  ...
  public void checkOutputSpecs(FileSystem ignored, JobConf job) 
    ...
    Path outDir = getOutputPath(job); // 获取文件输出路径
    if (outDir == null && job.getNumReduceTasks() != 0) {
      throw new InvalidJobConfException("Output directory not set in JobConf.");
    }
    if (outDir != null) {
      FileSystem fs = outDir.getFileSystem(job); // 文件系统
      // normalize the output directory
      outDir = fs.makeQualified(outDir); // 对输出目录进行规范化
      ...
      // check its existence
      if (fs.exists(outDir)) { // 如果文件输出路径存在，则抛出异常，任务终止
