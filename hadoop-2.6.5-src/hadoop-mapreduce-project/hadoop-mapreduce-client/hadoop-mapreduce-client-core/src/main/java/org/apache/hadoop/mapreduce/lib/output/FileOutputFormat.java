...
public static final String OUTDIR = "mapreduce.output.fileoutputformat.outputdir";
  ...
  public void checkOutputSpecs(JobContext job
    ...
    Path outDir = getOutputPath(job); // 文件输出路径
    if (outDir == null) { // 如果是null抛出异常，任务运行结束
      throw new InvalidJobConfException("Output directory not set.");
    }

    // get delegation token for outDir's file system
    TokenCache.obtainTokensForNamenodes(job.getCredentials(),
        new Path[] { outDir }, job.getConfiguration()); // 做一些文件系统的验证，非核心，不细看

    if (outDir.getFileSystem(job.getConfiguration()).exists(outDir)) { // 检查输出文件路径是否存在，mapreduce的文件输出路径是不能存在的；如果存在则抛出异常
