...
public class LocalJobRunner implements ClientProtocol {
  ...
  private class Job extends Thread implements TaskUmbilicalProtocol {
    ...
    public Job(JobID jobid, String jobSubmitDir) throws IOException {
      this.systemJobDir = new Path(jobSubmitDir); // 任务资源路径：job.xml、job.split、job.splitmetainfo等
      this.systemJobFile = new Path(systemJobDir, "job.xml"); // job.xml
      this.id = jobid; // job-id
      JobConf conf = new JobConf(systemJobFile);
      this.localFs = FileSystem.getLocal(conf);
      String user = UserGroupInformation.getCurrentUser().getShortUserName();
      this.localJobDir = localFs.makeQualified(new Path(
          new Path(conf.getLocalPath(jobDir), user), jobid.toString())); // 任务运行的路径：./local/localRunner/hdfs/job_local1045495247_0001
      this.localJobFile = new Path(this.localJobDir, id + ".xml"); // 任务运行的文件：./local/localRunner/hdfs/job_local1045495247_0001/job_local1045495247_0001.xml
      ...
      // updated it.
      OutputStream out = localFs.create(localJobFile); // 创建文件流
      try {
        conf.writeXml(out); // 将所有的配置信息写入job_local1045495247_0001.xml文件
      } finally {
        out.close();
      }
      this.job = new JobConf(localJobFile); // job的所有配置项
      ...
      profile = new JobProfile(job.getUser(), id, systemJobFile.toString(), 
                               "http://localhost:8080/", job.getJobName()); // 设置任务的一些web信息
      /**
       * job-id : job_local1045495247_0001uber-mode : falsemap-progress : 0.0reduce-progress : 0.0cleanup-progress : 0.0setup-progress : 0.0runstate : RUNNIN
       * Gstart-time : 0user-name : hdfspriority : NORMALscheduling-info : NAnum-used-slots0num-reserved-slots0used-mem0reserved-mem0needed-mem0
       * 
       * 这些信息用于在页面显示
       */
      ...
      jobs.put(id, this); // 加入缓存信息

      this.start(); // 本地运行任务启动；Job继承了Thread，就是运行run方法
    }
      ...
      public void run() {
        try {
          TaskAttemptID mapId = new TaskAttemptID(new TaskID(
              jobId, TaskType.MAP, taskId), 0); // 封装信息并生成map-task的id：attempt_local515674942_0001_m_000000_0
          LOG.info("Starting task: " + mapId);
          mapIds.add(mapId); // 加入缓存用来统计
          
          /**
           * systemJobFile.toString() ： ./.staging/job_local515674942_0001/job.xml
           * mapId : attempt_local515674942_0001_m_000000_0
           * taskId : 0(表示此task是第几次运行，可能此task会被kill或failed或运行慢，重试时这个taskId会增加)
           * info.getSplitIndex() : split信息
           */
          MapTask map = new MapTask(systemJobFile.toString(), mapId, taskId,
            info.getSplitIndex(), 1); // 创建map-task
          map.setUser(UserGroupInformation.getCurrentUser().
              getShortUserName()); // 设置使用者
          setupChildMapredLocalDirs(map, localConf); // 设置task运行文件输出目录

          MapOutputFile mapOutput = new MROutputFiles(); // map-task文件输出
          mapOutput.setConf(localConf);
          mapOutputFiles.put(mapId, mapOutput); // 全局存储：key map-task-id，value map-task文件输出封装对象(MapOutputFile)

          map.setJobFile(localJobFile.toString()); // 设置文件路径：./job_local515674942_0001/job_local515674942_0001.xml
          localConf.setUser(map.getUser()); // 设置用户
          map.localizeConfiguration(localConf);
          map.setConf(localConf);
          try {
            map_tasks.getAndIncrement(); // 数量增加
            myMetrics.launchMap(mapId); // 启动map-task
            map.run(localConf, Job.this); // 运行map-task
            myMetrics.completeMap(mapId); // 设置指标，完成的map-task
          } finally {
    ...
    protected List<RunnableWithThrowable> getMapTaskRunnables(
      ...
      ArrayList<RunnableWithThrowable> list =
          new ArrayList<RunnableWithThrowable>(); // RunnableWithThrowable集合：RunnableWithThrowable实现了Runnable接口的线程
      for (TaskSplitMetaInfo task : taskInfo) { // 迭代SplitMetaInfo，一个split就是一个MapTaskRunnable，MapTaskRunnable继承了RunnableWithThrowable，MapTaskRunnable就是一个线程
        list.add(new MapTaskRunnable(task, numTasks++, jobId,
            mapOutputFiles));
      }

      return list; // 返回封装了split的MapTaskRunnable线程集合
    }
    ...
    protected synchronized ExecutorService createMapExecutor() {
      ...
      int maxMapThreads = job.getInt(LOCAL_MAX_MAPS, 1); // 同时运行的线程数量
      if (maxMapThreads < 1) {
      ...
      ThreadFactory tf = new ThreadFactoryBuilder()
        ...
        .build();
      ExecutorService executor = Executors.newFixedThreadPool(maxMapThreads, tf); // 创建线程池
    ...
    private void runTasks(List<RunnableWithThrowable> runnables,
      ...
      for (Runnable r : runnables) { // 迭代封装的List<RunnableWithThrowable>
        service.submit(r); // 将他们一个一个的提交到线程池并运行，运行的是MapTaskRunnable线程的run方法
      }
    ...
    @Override
    public void run() {
      JobID jobId = profile.getJobID(); // job-id
      ...
      try {
        TaskSplitMetaInfo[] taskSplitMetaInfos = 
          SplitMetaInfoReader.readSplitMetaInfo(jobId, localFs, conf, systemJobDir); // Split集合信息，加载job.split来获得

        int numReduceTasks = job.getNumReduceTasks(); // reduce-task的数量，如果是0的话，就不会启动reduce-task，就不会有shuffer
        outputCommitter.setupJob(jContext); // 设置配置信息
        status.setSetupProgress(1.0f);

        Map<TaskAttemptID, MapOutputFile> mapOutputFiles =
            Collections.synchronizedMap(new HashMap<TaskAttemptID, MapOutputFile>()); // map输入文件记录
        
        /**
         * 这里是关键的地方，根据taskSplitMetaInfos、jobId、mapOutputFiles来构建MapTaskRunnables
         * 其实就是通过splits来构建一个一个的thread，处理split分到的数据
         */
        List<RunnableWithThrowable> mapRunnables = getMapTaskRunnables(
            taskSplitMetaInfos, jobId, mapOutputFiles);
              
        initCounters(mapRunnables.size(), numReduceTasks); // map-task计数器
        ExecutorService mapService = createMapExecutor(); // 创建运行map-task任务的线程池
        runTasks(mapRunnables, mapService, "map"); // 开始运行map-task
  ...
  public org.apache.hadoop.mapreduce.JobStatus submitJob(
    ...
    Job job = new Job(JobID.downgrade(jobid), jobSubmitDir); // 创建job
  ...
  public String getStagingAreaDir() throws IOException {
    Path stagingRootDir = new Path(conf.get(JTConfig.JT_STAGING_AREA_ROOT, 
        "/tmp/hadoop/mapred/staging")); // 临时文件夹
    ...
    return fs.makeQualified(new Path(stagingRootDir, user+"/.staging")).toString(); // 查检并返回任务的临时文件存放路径
  }
  ...
  static void setupChildMapredLocalDirs(Task t, JobConf conf) {
    String[] localDirs = conf.getTrimmedStrings(MRConfig.LOCAL_DIR); // [/tmp/hadoop-Administrator/mapred/local]
    String jobId = t.getJobID().toString(); // job_local515674942_0001
    String taskId = t.getTaskID().toString(); // attempt_local515674942_0001_m_000000_0
    boolean isCleanup = t.isTaskCleanupTask(); // false
    String user = t.getUser(); // 用户hdfs
    
    /**
     * 拼接map-task运行文件路径
     * /tmp/hadoop-Administrator/mapred/local/localRunner//hdfs/jobcache/job_local515674942_0001/attempt_local515674942_0001_m_000000_0
     */
    ...
    conf.set(MRConfig.LOCAL_DIR, childMapredLocalDir.toString()); // 设置配置项
  }
